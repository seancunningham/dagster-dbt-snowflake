...
class DagsterSlingFactory:
        ...
        for config_path in os.listdir(config_dir):
            if config_path.endswith(".yaml") or config_path.endswith(".yml"):
                ...
                if connection_configs := config.get("connections"):
                    connections, kind_map = (
                        DagsterSlingFactory._get_connections(
                            connection_configs,
                            connections,
                            kind_map)
                    )
                ...
                if replication_configs := config.get("replications"):
                    assets, freshness_checks = (
                        DagsterSlingFactory._get_replications(
                            replication_configs,
                            freshness_checks,
                            kind_map,
                            assets)
    ...
    def _get_connections(
        ...
        """Returns a list of SlingConnectionResource for
        connections in the Sling yaml file
        """
    ...
    @staticmethod
    def _get_connection(connection_config: dict
                        ) -> SlingConnectionResource | None:
        """Returns a SlingConnectionResource for
        a connection in the Sling yaml file
        """
    ...
    def _get_replications(
            ...
            freshness_checks,
            kind_map,
            assets) -> tuple[list[dg.AssetsDefinition], list[dg.AssetChecksDefinition]]:
        ...
        for replication_config in replication_configs:
            if bool(os.getenv("DAGSTER_IS_DEV_CLI")):
                replication_config = DagsterSlingFactory._set_dev_schema(replication_config)
            ...
            dep_asset_specs = DagsterSlingFactory._get_sling_deps(replication_config, kind)
            asset_freshness_checks = DagsterSlingFactory._get_freshness_checks(replication_config)
    ...
    def _get_nested(config: dict, path: list) -> Any:
        """Helper function to safely traverse a nested dictionary
        that may have null values for a set key that is expected
        to be a dict.
        helpful because stream definitions that use only the default
        configs behave this way.
    ...
    def _get_freshness_checks(replication_config: dict) -> list[dg.AssetChecksDefinition]:
        ...
        default_freshness_check_config = DagsterSlingFactory._get_nested(replication_config, ["defaults", "meta", "dagster", "freshness_check"]) or {}
        default_partition = DagsterSlingFactory._get_nested(replication_config, ["defaults", "meta", "dagster", "partition"])

        streams = replication_config.get("streams", {})
        for stream_name, steam_config in streams.items():
            freshness_check_config = DagsterSlingFactory._get_nested(steam_config, ["meta", "dagster", "freshness_check"]) or {}
            partition = DagsterSlingFactory._get_nested(steam_config, ["meta","dagster","partition"])
